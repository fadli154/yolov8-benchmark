...
public class Yolo {
    ...
    protected String activeBackendStatus = "CPU active";
    protected String activeBackendType = "CPU";
    protected GpuDelegate gpuDelegate;
    protected NnApiDelegate nnApiDelegate;
    ...
    public String getActiveBackendStatus() {
        return this.activeBackendStatus;
    }

    public String getActiveBackendType() {
    ...
    public void initialize_model() throws Exception {
        ...
        try {
            ...
            String selectedBackend = this.backend;
            if (selectedBackend == null || selectedBackend.trim().isEmpty()) {
                selectedBackend = this.use_gpu ? "GPU" : "CPU";
            }

            boolean success = false;
            String status = "CPU active";
            String activeType = "CPU";

            if ("GPU".equalsIgnoreCase(selectedBackend)) {
                try {
                    CompatibilityList compatibilityList = new CompatibilityList();
                    if (compatibilityList.isDelegateSupportedOnThisDevice()) {
                        GpuDelegateFactory.Options delegateOptions = compatibilityList.getBestOptionsForThisDevice();
                        this.gpuDelegate = new GpuDelegate(delegateOptions.setQuantizedModelsAllowed(this.quantization));
                        interpreterOptions.addDelegate(this.gpuDelegate);
                        this.interpreter = new Interpreter(buffer, interpreterOptions);
                        success = true;
                        status = "GPU delegate active";
                        activeType = "GPU";
                    } else {
                        status = "GPU unavailable, fallback to CPU";
                    }
                } catch (Exception e) {
                    Log.e("Yolo", "GPU delegate creation failed, falling back to CPU: " + e.getMessage());
                    status = "GPU unavailable, fallback to CPU";
                    if (this.gpuDelegate != null) {
                        try {
                            this.gpuDelegate.close();
                        } catch (Exception ex) {
                            Log.e("Yolo", "Error closing failed GpuDelegate", ex);
                        }
                        this.gpuDelegate = null;
                    }
                }
            } else if ("NNAPI".equalsIgnoreCase(selectedBackend)) {
                try {
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.P) {
                        NnApiDelegate.Options nnapiOptions = new NnApiDelegate.Options();
                        this.nnApiDelegate = new NnApiDelegate(nnapiOptions);
                        interpreterOptions.addDelegate(this.nnApiDelegate);
                        this.interpreter = new Interpreter(buffer, interpreterOptions);
                        success = true;
                        status = "NNAPI active";
                        activeType = "NNAPI";
                    } else {
                        status = "NNAPI unavailable, fallback to CPU";
                    }
                } catch (Exception e) {
                    Log.e("Yolo", "NNAPI delegate creation failed, falling back to CPU: " + e.getMessage());
                    status = "NNAPI unavailable, fallback to CPU";
                    if (this.nnApiDelegate != null) {
                        try {
                            this.nnApiDelegate.close();
                        } catch (Exception ex) {
                            Log.e("Yolo", "Error closing failed NnApiDelegate", ex);
                        }
            ...
            if (!success) {
                ...
                interpreterOptions.setNumThreads(this.num_threads);
                this.interpreter = new Interpreter(buffer, interpreterOptions);
                activeType = "CPU";
                if ("CPU".equalsIgnoreCase(selectedBackend)) {
                    status = "CPU active";
                }
            }

            this.activeBackendStatus = status;
            this.activeBackendType = activeType;
    ...
    public void close() {
        try {
            ...
            Log.e("Yolo", "Interpreter or delegate close error", e);
        }
    }
}
