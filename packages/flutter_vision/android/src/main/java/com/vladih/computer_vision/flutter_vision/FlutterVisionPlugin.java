...
public class FlutterVisionPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    private void loadYoloModel(Map<String, Object> args, Result result) {
        try {
            ...
            final String backend = args.containsKey("backend") ? (String) args.get("backend") : (useGpu ? "GPU" : "CPU");
            ...
            Log.d(TAG, String.format("Loading YOLO model: version=%s, threads=%d, gpu=%b, backend=%s, quantized=%b", 
                    version, numThreads, useGpu, backend, quantization));
            ...
            // Set target backend
            yolo_model.setBackend(backend);
            
            // Initialize the model
            yolo_model.initialize_model();
            
            Log.d(TAG, "YOLO model loaded successfully: " + version + " using backend " + yolo_model.getActiveBackendStatus());
            
            java.util.Map<String, Object> response = new java.util.HashMap<>();
            response.put("activeBackend", yolo_model.getActiveBackendType());
            response.put("activeBackendStatus", yolo_model.getActiveBackendStatus());
